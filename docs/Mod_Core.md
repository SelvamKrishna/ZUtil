...
Provides ANSI escape sequences for terminal text formatting Works seamlessly with
`std::ostream` and `std::format` by producing ANSI escape sequences. Can be globally
disabled via `Z_DISABLE_ANSI`.
...
`ProString` encapsulates a string with an associated ANSI color code, providing convenient
colored output for logging and debugging.
...
Enhanced assertion system with colored output and source location tracking. Disabled in release builds
...
| Function                                                    | Description                             |
| ----------------------------------------------------------- | --------------------------------------- |
| `void Assert(bool condition, const ProString& description)` | Assert with optional always-on behavior |
